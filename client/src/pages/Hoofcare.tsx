...
function HoofcareContent() {
  ...
  const getConditionBadge = (condition: string) => {
    ...
    return <Badge className={variants[condition] || variants.good}>{condition}</Badge>;
  };

  const upcomingRecords = localRecords?.filter((record: any) => {
    ...
    <div className="container mx-auto py-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-muted-foreground">Manage farrier visits and hoof maintenance</p>
        </div>
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editingRecord ? 'Edit Hoofcare Record' : 'New Hoofcare Record'}</DialogTitle>
              <DialogDescription>
                {editingRecord ? 'Update hoofcare details' : 'Record a new farrier visit'}
              </DialogDescription>
            </DialogHeader>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="grid gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setFormData({...formData, farrierName: e.target.value})}
                      required
                    />
                  </div>
                  <div className="grid gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setFormData({...formData, farrierContact: e.target.value})}
                    />
                  </div>
                </div>
        ...
        <Card className="mb-6 border-blue-200 bg-blue-50">
          <CardHeader>
            ...
            <CardDescription className="text-blue-700">
              {upcomingRecords.length} visit{upcomingRecords.length !== 1 ? 's' : ''} due in the next 30 days
            </CardDescription>
          </CardHeader>
        </Card>
      )}
        ...
        {localRecords && localRecords.length > 0 ? (
              ...
              <CardHeader>
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <CardDescription>
                      {new Date(record.date).toLocaleDateString()} • {record.farrierName}
                    </CardDescription>
                  </div>
              ...
              <CardContent>
                <div className="grid gap-2 text-sm">
                  ...
                  {record.workPerformed && (
                    <div>
                      <span className="font-medium">Work Performed:</span> {record.workPerformed}
                    </div>
                  )}
                  {record.findings && (
                    <div>
          ...
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-muted-foreground">No hoofcare records yet. Add your first record above.</p>
            </CardContent>
          </Card>
        )}
      </div>
