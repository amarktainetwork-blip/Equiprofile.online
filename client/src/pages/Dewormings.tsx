...
export default function Dewormings() {
  ...
  useRealtimeModule('dewormings', (action, data) => {
    if (action === 'created') {
      ...
      setLocalDewormings(prev => prev.map(item => item.id === data.id ? { ...item, ...data } : item));
        ...
        <div className="flex justify-between items-center">
          ...
          <Dialog open={isDialogOpen} onOpenChange={(open) => {
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>{editingId ? 'Edit Deworming' : 'Add Deworming'}</DialogTitle>
              </DialogHeader>
            ...
            <CardContent>
              <div className="space-y-2">
                {upcomingDewormings.map((deworming: any) => {
                  ...
                  return (
                    <div key={deworming.id} className="flex justify-between items-center p-2 bg-white rounded">
                      <div>
                        <span className="font-medium">{horse?.name}</span> - {deworming.product}
                      </div>
              ...
              <Card key={deworming.id}>
                <CardHeader>
                  <CardTitle className="flex justify-between items-start">
                    <div>
                      <div className="text-lg">{horse?.name || 'Unknown Horse'}</div>
                      <div className="text-sm font-normal text-gray-600">{deworming.product}</div>
                    </div>
                    <div className="flex gap-2">
                      <Button size="sm" variant="ghost" onClick={() => handleEdit(deworming)}>
                        <Edit className="h-4 w-4" />
                      </Button>
                      <Button size="sm" variant="ghost" onClick={() => handleDelete(deworming.id)}>
                ...
                <CardContent className="space-y-2">
                  <div className="text-sm">
                    <span className="text-gray-600">Active Ingredient:</span> {deworming.activeIngredient}
                  </div>
                  <div className="text-sm">
                    <span className="text-gray-600">Dosage:</span> {deworming.dosage}
                  </div>
                  {deworming.weight && (
                    <div className="text-sm">
                      <span className="text-gray-600">Weight:</span> {deworming.weight} kg
                    </div>
                  )}
                  <div className="text-sm">
                    <span className="text-gray-600">Date:</span> {new Date(deworming.dateAdministered).toLocaleDateString()}
                  </div>
                  {deworming.nextDueDate && (
                    <div className="text-sm">
                      <span className="text-gray-600">Next Due:</span> {new Date(deworming.nextDueDate).toLocaleDateString()}
                    </div>
                  )}
                  {deworming.vet && (
                    <div className="text-sm">
                      <span className="text-gray-600">Vet:</span> {deworming.vet}
                    </div>
                  )}
                  {deworming.cost && (
                    <div className="text-sm">
                      <span className="text-gray-600">Cost:</span> £{(deworming.cost / 100).toFixed(2)}
                    </div>
                  )}
                  {deworming.notes && (
                    <div className="text-sm">
                      <span className="text-gray-600">Notes:</span> {deworming.notes}
                    </div>
                  )}
                </CardContent>
              </Card>
        ...
        {localDewormings.length === 0 && (
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-sm text-gray-500">Click "Add Deworming" to create your first record</p>
            </CardContent>
          </Card>
        )}
      </div>
