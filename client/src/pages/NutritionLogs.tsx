...
function NutritionLogsContent() {
  ...
  useRealtimeModule('nutritionLogs', (action, data) => {
    if (action === 'created') {
      ...
      setLocalLogs((prev) => prev.map((log) => (log.id === data.id ? { ...log, ...data } : log)));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        waterConsumption: formData.waterConsumption ? parseFloat(formData.waterConsumption) : undefined,
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <Dialog open={isDialogOpen} onOpenChange={(open) => { setIsDialogOpen(open); if (!open) resetForm(); }}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editingLog ? 'Edit' : 'Add'} Nutrition Log</DialogTitle>
            </DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormData({ ...formData, nightFeed: e.target.value })}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormData({ ...formData, hayAmount: e.target.value })}
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  <Label htmlFor="bodyConditionScore">Body Condition (1-9)</Label>
                  <Select value={formData.bodyConditionScore} onValueChange={(value) => setFormData({ ...formData, bodyConditionScore: value })}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
              ...
              <div className="flex gap-2 justify-end">
                ...
                <Button type="submit">{editingLog ? 'Update' : 'Create'} Log</Button>
              </div>
            </form>
          </DialogContent>
        </Dialog>
            ...
            <Card key={log.id}>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-lg font-medium">
                  {horses?.find((h: any) => h.id === log.horseId)?.name || 'Horse'} - {log.date}
                </CardTitle>
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  {log.feedType && <div><span className="font-medium">Feed:</span> {log.feedType} {log.feedAmount && `(${log.feedAmount} kg)`}</div>}
                  {log.breakfast && <div><span className="font-medium">Breakfast:</span> {log.breakfast}</div>}
                  {log.lunch && <div><span className="font-medium">Lunch:</span> {log.lunch}</div>}
                  {log.dinner && <div><span className="font-medium">Dinner:</span> {log.dinner}</div>}
                  {log.nightFeed && <div><span className="font-medium">Night Feed:</span> {log.nightFeed}</div>}
                  {log.supplements && <div><span className="font-medium">Supplements:</span> {log.supplements}</div>}
                  {log.hayAmount && <div><span className="font-medium">Hay:</span> {log.hayAmount}</div>}
                  {log.waterConsumption && <div><span className="font-medium">Water:</span> {log.waterConsumption} L</div>}
                </div>
